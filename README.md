...
- 当前这档移速增强属于 clientOnly 实现，在部分联机场景下仍可能碰到同步边界或轻微回拉。
...
- 本地移速增强逻辑集中在 `scripts/widgets/clientspeedwidget.lua`。
...
- 当前移速增强通过玩家身上的 `playerspeedmult:SetPredictedSpeedMult(...)` 挂一个固定来源键实现。
...
- It also adds a `1.05x` movement speed boost.
...
- The current speed boost is implemented through a client-only mod, so some multiplayer setups can still hit sync boundaries or mild rubber-banding.
...
- The local move-speed logic lives in `scripts/widgets/clientspeedwidget.lua`.
...
- The current speed boost is implemented through `playerspeedmult:SetPredictedSpeedMult(...)` with a fixed local source key.
