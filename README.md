...
- It shows local text markers around your character for nearby butterflies, players, Touch Stones, and long-range Pig King tracking.
- Each marker spreads by direction and includes rough distance text, while Touch Stones and Pig King can also keep working through session-memory caching.
...
Notes:
- This project will keep growing with more practical helper features later, but the current focus is stabilizing the HUD guidance, player list, and client-side experiments.
- The current feature only draws local HUD indicators and does not modify server-side entities or affect other players.
- Flower tracking has now been removed because nearby butterflies already act as a decent proxy for nearby flowers and showing both mostly created clutter.
- The current guidance tracks nearby `butterfly`, living players, and `resurrectionstone`, while Pig King can also be tracked at long range.
...
- Your own exact latency is shown as `xx ms`; if Lua does not expose exact milliseconds for other players, their rows fall back to approximate bands like `50± ms / 200± ms / 999± ms` instead of staying at `-- ms`.
...
- Recorded issue: earlier `1.1x` and `1.05x` client-only move-speed experiments both produced rubber-banding during real play, so the movement-speed boost was removed.
- The interaction reach tweak is another client-only experiment focused on probing the acceptance boundary of local action-distance changes in multiplayer.
...
- Recorded issue: long-range Pig King guidance could not rely on `TheSim:FindEntities(...)` alone because the client does not know about unsynced faraway Pig King entities.
- Recorded issue: the first off-screen direction approximations also drifted too much once `TheSim:GetScreenPos(...)` stopped being usable; switching to `TheCamera:GetRightVec()` and `GetDownVec()` for approximation ended up behaving much better.
...
- Nearby target lookup mainly uses `TheSim:FindEntities(...)`, now split into butterflies, players, Touch Stones, and Pig King to avoid a broad unfiltered radius-100 scan.
- Butterflies use `butterfly`, players use `player` while excluding `playerghost`, Touch Stones use `resurrector`, and Pig King also uses a `TheWorld.topology` fallback for long range.
- Query results now keep only a stable distance-based order; there is no angle filter and no fixed display cap anymore.
- Screen-space placement prefers `TheSim:GetScreenPos(...)`, with camera-vector approximation through `TheCamera:GetRightVec()` / `GetDownVec()` and the older heading path kept only as a fallback.
