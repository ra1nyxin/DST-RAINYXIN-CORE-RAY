...
- 已记录：2026-07-24 这版菜单后续又出现“只有 Insert 显隐有反应，鼠标点击和拖动完全没反应”的问题，根因是自建 `ImageButton(blank.tex)` 之后只设了图片尺寸和透明度，但没显式 `SetClickable(true)`，前端鼠标命中阶段不会把它识别成可交互 widget；这类命中按钮要同时给按钮本体和必要的图片层补 clickable。
- 已记录：2026-07-24 继续追菜单输入时，发现 DST 这套 HUD 里“自定义透明按钮 + 焦点链 + `OnControl` 主键映射”对这种纯代码大面板并不稳定，容易出现“只有第一项正常、其他项不响应、拖动也失效”的状态；这类自定义菜单更稳的做法是直接挂全局 `AddMouseButtonHandler` / `AddMoveHandler` / `AddGeneralControlHandler`，自己做标题区、列表项、滚轮的命中检测。
- 已记录：2026-07-24 第一版全局鼠标处理虽然回调已正确挂上，但把 `AddMouseButtonHandler` 的第二个参数误当成 `down`；原版输入桥实际传的是 `is_up`，即按下为 `false`、松开为 `true`。这会让列表在松开时才开始记录按下、让标题在松开时才开始拖动，表面上就是全部列表项和拖动都没有反应。后续处理该回调必须明确按 `is_up` 语义分支。
- 已记录：2026-07-24 后续确认 HUD 的全局鼠标回调在这个挂载层不可靠，不能作为菜单唯一输入来源。菜单改回原版 UI 命中链：用实际 `ImageButton` 处理焦点和点击，并让其 `blank.tex` 命中图片保留 alpha 为 `1`、`focus_forward` 指回按钮本体。若把命中图片 alpha 设为 `0`，引擎可能直接跳过鼠标射线命中，外观仍正常但所有鼠标交互都会消失。
- 已记录：2026-07-24 这套按钮命中链中，`ImageButton` 根节点本身没有图片尺寸，不能同时 `SetClickable(true)`；否则后创建的无边界根节点可能覆盖前面所有列表项，实测会表现为“只有最后一项绿洲能高亮和点击”。可点击的只能是经过 `ForceImageSize(...)` 的 image 子节点，再用 `focus_forward` 转交给按钮本体。滚轮也应直接由当前获得焦点的按钮处理，避免依赖父 HUD 的控制分发。
- 现在没有做外部配置文件开关。
